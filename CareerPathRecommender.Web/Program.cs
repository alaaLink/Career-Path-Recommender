...
using (var scope = app.Services.CreateScope())
{
    ...
    Log.Information("Current employee count: {EmployeeCount}", totalEmployees);
    
    if (totalEmployees == 0)
    {
        Log.Information("Starting database seeding with 50 employees...");
        await SeedDataAsync(employeeRepository, courseRepository, projectRepository, skillRepository);
        Log.Information("Database seeding completed successfully.");
    }
    else
    {
        Log.Information("Database already contains {EmployeeCount} employees. Skipping seeding.", totalEmployees);
    }
}
...
async Task SeedDataAsync(IEmployeeRepository employeeRepository, ICourseRepository courseRepository, IProjectRepository projectRepository, ISkillRepository skillRepository)
{
    ...
    // Create scope for ProjectSkills
